...
```{r mrtrashwheel}
mrtrash_df =
  read_excel("202509 Trash Wheel Collection Data.xlsx", sheet = 1, range = "A2:N709", col_names = TRUE) %>% 
  janitor::clean_names() %>% 
  drop_na(dumpster) %>% 
  mutate(
...
```{r proftrash}
proftrash_df =
  read_excel("202509 Trash Wheel Collection Data.xlsx", sheet = 2, range = "A2:M134", col_names = TRUE) %>% 
  janitor::clean_names() %>% 
  drop_na(dumpster) %>% 
  mutate(
...
```{r gwynnda}
gwynnda_df =
  read_excel("202509 Trash Wheel Collection Data.xlsx", sheet = 4, range = "A2:L351", col_names = TRUE) %>% 
  janitor::clean_names() %>%
  drop_na(dumpster) %>% 
  mutate(
